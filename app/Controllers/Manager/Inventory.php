...
class Inventory extends BaseController
{
    ...
    public function store()
    {
        $data = [
            ...
            'processor_full_name'   => $this->request->getPost('processor_full_name'),
            ...
            'internet_location'    => $this->request->getPost('internet_location'),
            'sections'             => $this->request->getPost('sections'),
            'location'             => $this->request->getPost('location'),
            'warranty_information' => $this->request->getPost('warranty_information'),
            'year_of_manufacture'  => $this->request->getPost('year_of_manufacture'),
            'expired_year'         => $this->request->getPost('expired_year'),
            'expired_date'         => $this->request->getPost('expired_date'),
            'notes'                => $this->request->getPost('notes'),
            'registered_by'        => $this->request->getPost('registered_by'),
            'registered_date'      => $this->request->getPost('registered_date'),
            'checked_by'           => $this->request->getPost('checked_by'),
            'checked_date'         => $this->request->getPost('checked_date')
        ];
        ...
        if ($photo && $photo->isValid() && !$photo->hasMoved()) {
            ...
            $data['photo'] = 'uploads/inventory/' . $newName;
        }

        $this->inventory->insert($data);

        return redirect()->to('/manager/inventory')->with('success', 'Inventory added successfully');
    }

    public function edit($id)
    {
        $data['item'] = $this->inventory->find($id);

        if (!$data['item']) {
            return redirect()->to('/manager/inventory')->with('error', 'Item not found');
        }

        return view('manager/inventory/edit', $data);
    }
    ...
    public function update($id)
    {
        ...
        if ($photo && $photo->isValid() && !$photo->hasMoved()) {
            if (!empty($item['photo']) && file_exists(FCPATH . $item['photo'])) {
        ...
        return redirect()->to('/manager/inventory')->with('success', 'Inventory updated successfully');
    }
    ...
    public function delete($id)
    {
        ...
        if ($item) {
            ...
            return redirect()->to('/manager/inventory')->with('success', 'Inventory deleted successfully');
        }
